...
// Updates the 'last active' timestamp of a server.
func (db *Database) UpdateServerLastActive(serverID int64) {
	db.mutex.Lock()
	defer db.mutex.Unlock()

	_, err := db.Exec("update `servers` set `last_active` = strftime('%s', 'now') where `id` = ?", serverID)
	if err != nil {
		log.Fatalln("error updating server's 'last active' timestamp:", err)
	}
}
...
func (db *Database) FindServerByDescription(desc string) []Server {
	...
	for rows.Next() {
		s := Server{}
		rows.Scan(&s.ID, &s.IP, &s.Port, &s.Description, &s.Mod, &s.LastUpdated)
